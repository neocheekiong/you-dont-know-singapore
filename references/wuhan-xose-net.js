    ...
    if (data[i]['caseType'] == 'newCase') {
      ...
      if (data[i]['mohURL'] == '') {
        L.marker([data[i]['lat'], data[i]['lng']], {
          ...
          idx: i,
          ridx: data[i]['relatedArrayNo']
        }).bindPopup('<b>Related To Case:</b> ' + data[i]['relatedCaseNo'] + '<br><b>Location:</b> ' + data[i]['location'])
          .addTo(map)
          .on('click', function (e) {
            toggleHideMarkers(e.target.options.idx, e.target.options.ridx);
          });
        ...
        L.marker([data[i]['lat'], data[i]['lng']], {
          ...
          idx: i,
          ridx: data[i]['relatedArrayNo']
        }).bindPopup('<b>Related To Case:</b> ' + data[i]['relatedCaseNo'] + '<br><b>Location:</b> ' + data[i]['location'] + '<br><b>Source: <a href=\'https://www.moh.gov.sg/news-highlights/details/' + data[i]['mohURL'] + '\' target=\'_blank\'>Ministry Of Health</a></b>')
          .addTo(map)
          .on('click', function (e) {
            toggleHideMarkers(e.target.options.idx, e.target.options.ridx);
          });
...
function toggleHideMarkers (markerIdx, relatedIdx) {
  if (markerIdx == '') {
    ...
    $('.leaflet-marker-pane > div').eq(markerIdx)
      .siblings()
      .css('opacity', '0.1');
  }
  if (relatedIdx != '') {
    ...
    for (let i = 0; i < rIdx.length; i++) {
      $('.leaflet-marker-pane > div').eq(parseInt(rIdx[i]))
        .css('opacity', '1');
      try {
